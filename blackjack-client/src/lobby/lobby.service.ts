...
export class LobbyService {
  ...
  public getGames(): Observable<Game[]> {
    return Axios.get(`https://localhost/blackjackapi/api/lobby/list`)
  ...
  public createGame(request: CreateGameRequest): Observable<string> {
    return Axios.post(`https://localhost/blackjackapi/api/lobby/newgame/${request.gameName}/${request.minBet}/${request.maxBet}`)
      .pipe(map((response: AxiosResponse<string>) => response.data as string));
  }

  public deleteGame(gameId: string): Observable<any> {
    return Axios.delete(`https://localhost/blackjackapi/api/lobby/deleteGame/${gameId}`);
  }
}
